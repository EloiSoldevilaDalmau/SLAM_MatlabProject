...
factor{1+5} = struct(...
    'type', 'lmk',...
    'measurement', [1.2; 30*torad],... %al ser tipu lmk només necessitem 2 mesures
    'covariance', diag([1.5e-2, (10*pi/180)*2]),...
    'index', [1,4]);
...
factor{1+8} = struct(...
    'type', 'lmk',...
    'measurement', [2.5; -28*torad],... %al ser tipu lmk només necessitem 2 mesures
    'covariance', diag([2e-2, (7*pi/180)*2]),...
    'index', [0,6]);

factor{1+9} = struct(...
    'type', 'lmk',...
    'measurement', [1.7; -35*torad],... %al ser tipu lmk només necessitem 2 mesures
    'covariance', diag([1.5e-2, (10*pi/180)*2]),...
    'index', [1,6]);

factor{1+10} = struct(...
    'type', 'lmk',...
    'measurement', [0.8; -40*torad],... %al ser tipu lmk només necessitem 2 mesures
